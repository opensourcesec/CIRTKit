...
from lib.common.utils import string_clean
...
WILDFIRE_TOKEN = ""
WILDFIRE_URL = 'https://wildfire.paloaltonetworks.com/publicapi/'


class Reports(Module):
    cmd = 'reports'
    description = 'Online Sandboxes Reports'
    authors = ['emdel', 'nex', 'deralexxx', 'byt3smith']
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--wildfire', action='store_true', help='Find reports on Palo Alto Wildfire')
    ...
    def wildfire(self):
        ...
        url = WILDFIRE_URL + 'get/report'
        api = WILDFIRE_TOKEN
        if len(api) < 1:
            api = getpass.getpass('Wildfire API Token: ')
        params = {'hash':__sessions__.current.file.md5,'apikey':api, 'format':'xml'} # HTTP POST parameters
        r = requests.post(url, data=params) # Create request
        if r.raise_for_status() is None: # Error checking by HTTP status code
            print r.content # Write downloaded content to console
        else:
            self.log('info', "No reports for opened file")
        return

    def usage(self):
        self.log('', "Usage: reports <malwr|anubis|threat|joe|meta|wildfire>")
